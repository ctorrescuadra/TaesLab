function sol=ExergyCostCalculator(model,varargin)
...
%
    ...
    if(model.NrOfWastes>0)
	    ...
	    ect=cExergyCost(rex,wt);
    else
        ect=cExergyCost(rex);
    end
    if ~ect.isValid
		ect.printLogger;
		ect.printError('Invalid Exergy Cost Computation. See error log')
		return
    end
	...
	sol=getExergyCostResults(fmt,ect,param);
