...
%
	...
	methods (Access=private)
        ...
        % Create Figure Components
            ...
            % Tool Bar
			tb = uitoolbar(hf);
			app.tb_ps = uipushtool (tb, 'cdata', cType.getIcon(cType.ResultId.PRODUCTIVE_STRUCTURE),...
				'tooltipstring','Show Productive Structure',...
				'clickedcallback', @(src,evt) app.productiveStructure(src,evt));
			app.tb_ts = uipushtool (tb, 'cdata', cType.getIcon(cType.ResultId.THERMOECONOMIC_STATE),...
			    'tooltipstring','Show Thermoeconomic State',...
			    'clickedcallback', @(src,evt) app.thermoeconomicState(src,evt));
            app.tb_ta = uipushtool (tb, 'cdata', cType.getIcon(cType.ResultId.THERMOECONOMIC_ANALYSIS),...
			    'tooltipstring','Thermoeconomic Analysis',...
			    'clickedcallback', @(src,evt) app.thermoeconomicAnalysis(src,evt));
            app.tb_td = uipushtool (tb, 'cdata', cType.getIcon(cType.ResultId.THERMOECONOMIC_DIAGNOSIS),...
			    'tooltipstring','Thermoeconomic Diagnosis',...
			    'clickedcallback', @(src,evt) app.thermoeconomicDiagnosis(src,evt));
            app.tb_ra = uipushtool (tb, 'cdata', cType.getIcon(cType.ResultId.WASTE_ANALYSIS),...
			    'tooltipstring','Waste Analysis',...
			    'clickedcallback', @(src,evt) app.wasteAnalysis(src,evt));
            app.tb_gs = uipushtool (tb, 'cdata', cType.getIcon(cType.ResultId.SUMMARY_RESULTS),...
			    'tooltipstring','General Summary',...
			    'clickedcallback', @(src,evt) app.summaryResults(src,evt));
            app.tb_mr = uipushtool (tb, 'cdata', cType.getIcon(cType.ResultId.RESULT_MODEL),...
