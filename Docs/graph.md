# Input-Output Analysis and Graph theory
...
1. $B \geq 0$ since $e_{ij} \geq 0$ and $x_i > 0$
2. Each row $i$ satisfies $\sum_{j=1}^n \beta_{ij} \leq 1$

The SSR property ensures that $I-B$ is Weakly Chained Diagonally Dominant (WCDD) because:
